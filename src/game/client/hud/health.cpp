...
void CHudHealth::Draw(float flTime)
{
	...
	gHUD.GetHudColor(HudPart::Health, m_iHealth, r, g, b);
	ScaleColors(r, g, b, a);
...
void CHudHealth::DrawPain(float flTime)
{
	...
	if (m_fAttackFront > 0.4)
	{
		...
		m_fAttackFront = max(0.f, m_fAttackFront - fFade);
	}
	else
		m_fAttackFront = 0;
	...
	if (m_fAttackRight > 0.4)
	{
		...
		m_fAttackRight = max(0.f, m_fAttackRight - fFade);
	}
	else
		m_fAttackRight = 0;
	...
	if (m_fAttackRear > 0.4)
	{
		...
		m_fAttackRear = max(0.f, m_fAttackRear - fFade);
	}
	else
		m_fAttackRear = 0;
